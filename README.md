...
このロールではlookupでpipeを呼び出してdateコマンドを実行しています。
...
ios_commandモジュールで複数のコマンドを打ち込むと、結果はstdout配列に格納されて戻ってきます。
この戻り値に実行したコマンドは含まれていません。
なのでstdout配列の中身だけをベタに書き込むと、わけがわからなくなります。
